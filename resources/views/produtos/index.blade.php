...
<div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
        ...
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-3xl font-bold text-gray-800 dark:text-gray-100">Lista de Produtos</h1>
        ...
        <form method="GET" action="{{ route('produtos.index') }}" class="mb-6">
            <div class="flex flex-wrap gap-4 items-end">
                ...
                <div>
                <label for="descricao" class="block text-sm font-bold text-gray-800 dark:text-gray-100  ">Descrição</label>
                    <input type="text" name="descricao" id="descricao" value="{{ request('descricao') }}"
                           class="mt-1 block w-full border-blue-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                </div>
                <div>
                    <button type="submit"
            ...
            <table class="min-w-full table-auto">
                <thead class="bg-gray-50 text-gray-600">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left font-medium text-sm text-gray-600 uppercase tracking-wider">Descrição</th>
                        <th class="px-6 py-3 text-left font-medium text-sm text-gray-600 uppercase tracking-wider">Preço</th>
                    ...
                    @foreach ($produtos as $produto)
                        <tr class="border-t border-b hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 flex items-center space-x-3">
                                <!--Botão Editar-->
                                <a href="{{ route('produtos.edit', $produto) }}" class="text-blue-500 hover:text-blue-700 mr-4">
                                ...
                                <form action="{{ route('produtos.destroy', $produto) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-500 hover:text-red-700 ml-4" onclick="return confirm('Tem certeza que deseja excluir este produto?')">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
